...
public class MongoFactory implements IFactory {
	...
	private iDAO2 mongoDAO; 

	@Override
	public Boolean startTransaction() {
		try {
			
			JarFile jarFile = new JarFile("assets/mongo.jar"); //Gets the jar
			Enumeration<JarEntry> e = jarFile.entries(); //Gets all the things inside the jar

			URL[] urls = { new URL("jar:file:" + "assets/mongo.jar" + "!/") };
			...
			while (e.hasMoreElements()) { //Runs the loop
			    ...
			    if (className.equals("MongoDatabase.MongoDAO")) { //If it finds the DAO class,
